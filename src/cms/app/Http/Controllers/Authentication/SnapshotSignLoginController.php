...
class SnapshotSignLoginController extends Controller
{
    ...
    {
        return Organisation::where('id', $request->input('organisation_id'))
            ->firstOrFail();
    }

    private function getSnapshotFromRequest(Request $request): Snapshot
    {
        return Snapshot::where('id', $request->input('snapshot_id'))
            ->firstOrFail();
    }

    private function getUserFromRequest(Request $request): User
    {
        return User::where('id', $request->input('user_id'))
            ->firstOrFail();
    }
