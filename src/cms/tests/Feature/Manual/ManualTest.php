...
/**
 * The figures are generated by tools/screenshots and referenced by hand, so a
 * renamed file or a typo'd path yields a broken image in the app while every
 * other test stays green. Walk both layers and check the files are really
 * there.
 */
it('references only figures that exist on disk', function (): void {
    $manual = new Manual();
    $bodies = [];

    foreach ($manual->chapters() as $chapter) {
        foreach ($chapter->topics as $topic) {
            $bodies[$topic->id] = $topic->body;
        }
    }

    foreach ($manual->tasks() as $task) {
        $bodies[$task->id . ' intro'] = $task->intro;

        foreach ($task->steps as $i => $step) {
            $bodies[$task->id . ' step ' . ($i + 1)] = $step->body;
        }
    }

    foreach ($bodies as $where => $body) {
        preg_match_all('#\((/handleiding/[^)]+)\)#', $body, $matches);

        foreach ($matches[1] as $path) {
            expect(public_path(ltrim($path, '/')))->toBeFile(sprintf(
